        ...
        <div class="card-body">
            ...
            <form class="forms-sample" action="{{ route('admin.history.update', $histories->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label>Title</label>
                    <input type="text" class="form-control" name="title" placeholder="Enter history title" value="{{ old('title', $histories->title) }}">
                </div>

                <div class="form-group">
                    <label>Description</label>
                    <input type="text" class="form-control" name="description" placeholder="Enter history description" value="{{ old('description', $histories->description) }}">
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-group col-xs-12 mb-2">
                        <input type="text" class="form-control file-upload-info" value="{{ $histories->photo ?? '' }}" readonly>
...
document.getElementById('fileInput').addEventListener('change', function(e) {
    ...
    if (file) {
        ...
        fileNameInput.value = '{{ $histories->photo ?? '' }}';
        preview.src = '{{ $histories->photo ? asset('storage/' . $histories->photo) : '' }}';
        preview.style.display = '{{ $histories->photo ? 'block' : 'none' }}';
    }
});
</script>
@endsection
