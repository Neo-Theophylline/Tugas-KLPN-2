                            ...
                            @foreach ($testimonials as $hero)
                                <tr>
                                    ...
                                    <td class="align-middle text-wrap">
                                        @for ($i = 1; $i <= 5; $i++)
                                            @if ($i <= $hero->stars)
                                                <span class="display-4" style="color: gold;">&#9733;</span>
                                            @else
                                                <span class="display-4" style="color: gray;">&#9733;</span>
                                            @endif
                                        @endfor
                                    </td>
                                    <td class="align-middle">
                                        ...
                                        <div class="d-flex align-items-center justify-content-center mb-2">
                                            {{-- Tombol Edit --}}
                                            <a href="{{ route('admin.testimonial.edit', $hero->id) }}"
                                                class="btn btn-success btn-sm d-flex align-items-center mr-2">
                                                Edit <i class="typcn typcn-edit ml-1"></i>
                                            </a>
                                            {{-- Tombol Delete --}}
                                            <form action="{{ route('admin.testimonial.destroy', $hero->id) }}" method="POST"
...
document.querySelectorAll('.toggle-status').forEach((el) => {
    el.addEventListener('change', function() {
        ...
        fetch("{{ route('admin.testimonial.toggle') }}", {
            method: "POST",
            headers: {
